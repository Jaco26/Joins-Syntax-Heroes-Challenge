...
SELECT heroes.name, powers.description
FROM heroes
    JOIN heroes_powers
...
INSERT INTO powers
    (name, description)
VALUES
    ('Ginger Tea', 'Can make a mouth spicy and alive');
...
VALUES
    (
        (SELECT id
        FROM heroes
        WHERE heroes.name = 'Superman'),
        (SELECT id
        FROM powers
        WHERE powers.name = 'Ginger Tea'),
        5
);
...
SELECT powers.name, powers.description, AVG(power_level)
FROM powers
    JOIN heroes_powers
    ON powers.id = heroes_powers.power_id
